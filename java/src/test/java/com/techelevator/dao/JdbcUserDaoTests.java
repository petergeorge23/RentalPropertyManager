...
public class JdbcUserDaoTests extends BaseDaoTests {
    protected static final User USER_1 = new User(1, "user1", "user1", "ROLE_USER", "firstName", "lastName", "email");
    protected static final User USER_2 = new User(2, "user2", "user2", "ROLE_USER", "firstName", "lastName", "email");
    private static final User USER_3 = new User(3, "user3", "user3", "ROLE_USER", "firstName", "lastName", "email");
    ...
    public void create_user_with_null_username() {
        sut.create(null, USER_3.getPassword(), "ROLE_USER", "firstName", "lastName", "email");
    }

    @Test(expected = DataIntegrityViolationException.class)
    public void create_user_with_existing_username() {
        sut.create(USER_1.getUsername(), USER_3.getPassword(), "ROLE_USER", "firstName", "lastName", "email");
    }

    @Test(expected = IllegalArgumentException.class)
    public void create_user_with_null_password() {
        sut.create(USER_3.getUsername(), null, "ROLE_USER", "firstName", "lastName", "email");
    }

    @Test
    public void create_user_creates_a_user() {
        User newUser = new User(-1, "new", "user", "ROLE_USER", "firstName", "lastName", "email");

        boolean userWasCreated = sut.create(newUser.getUsername(), newUser.getPassword(), "ROLE_USER", "firstName", "lastName", "email");
